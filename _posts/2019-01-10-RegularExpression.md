...
| 5 | (?<=expression) | 匹配expression后的位置 | 零宽断言 |ES中不支持 |
| 6 | (?!expression) | 匹配后面不是跟expression的位置 | 零宽断言 | |
| 7 | (?<!expression) | 匹配前面不是expression的位置 | 零宽断言 |ES中不支持 |
| 8 | (?#comment) | 不对正则表达式产生影响，仅供注释时用 | 注释 |ES中不支持 | 

上表中的注释可以列举 IP 地址单项的例子：  

- "2[0-4]\d(?#200-249)|25[0-5]\(?#250-255)|[01]?\d\d?(?#0-199)" 可以筛选出 0~255之间的数字，每一块都有注释
...
将后向引用和零宽断言结合起来的实例:  
"(?<=<(\w+)>).\*(?=<\/\1>)" 可用于匹配HTML 简单标签标签中间的内容(不包含标签的前缀和后缀)，只能识别无自定义class 或者 id 的标签内容。
...
RegExp 另一个方法是 test()，用来对某个字符串检测，如果包含匹配结果，返回 true。即当exec() 返回结果不是 null 时，test() 返回均为true
